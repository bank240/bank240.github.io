...
let eLae862 = document.createElement("div");
eLae862.id = "eLae86";
eLae862.style.position = "absolute";
eLae862.zIndex = "99999999"
eLae862.style.color = "white";
eLae862.textAlign = "right";
eLae862.borderRadius = "4px";
eLae862.style.width = "240px";
eLae862.style.height = "326px";
eLae862.style.display = "none";
eLae862.innerHTML = "block";
eLae862.style.top = "20px";
eLae862.style.right = "50px";
document.getElementById("gameUI").appendChild(eLae862);
"use strict";
// setInterval(()=>CanvasRenderingContext2D.prototype.roundRect = CanvasRenderingContext2D.prototype._roundRect, 150);
...
setInterval(()=>{
    ...
    if (get("pusher")) {
       ...
       document.getElementById("eLae86").style.display = "block";
                    document.getElementById("eLae86").innerHTML = `
                        <div style="font-size: 14px;">
                        Auto-Insta: ALWAYS ON<br>
                        AutoPush: ${get("pusher")}<br>
                        BouneGuy: ${get("bounce")}<br>
                        Type Insta: ${get("counter") ? "Normal" : "Auto-Counter"}
                        </div>
                        `;
},80);
...
!(function (e) {
    ...
    function (e, t, n) {
        ...
        (Math.lerpAngle = function (e, t, n) {
            ...
            (CanvasRenderingContext2D.prototype.roundRect = window.hp),
        ...
        function vn() {
            ...
            ttt = Date.now()
            R && R.alive && r.send("c", O, R.buildIndex >= 0 ? pn() : (R.weaponIndex == R.weapons[0] ? 90**100 : null));
        }
        ...
        function Cn(e) {
            ...
            N.forEach(obj => {
                if (obj.sid == e && Math.hypot(obj.y - R.y, obj.x - R.x) < 150) {
                    placer(R.items[2])
                }
            });
            nt.disableBySid(e);
        ...
        function Wn(e, t, n) {
            ...
            if (_ == R) {
                ...
                if (Math.hypot(R.x - _.x, R.y - _.y) < 150) {
                        for (let i = 0; i < Math.PI; i+=Math.PI/3) placer(R.items[2], i);
                        r.send("33", Math.atan2(_.y - R.y, _.x - R.x));
                        placer(R.items[2], R.dir - 1.5);
                        placer(R.items[2], R.dir - Math.PI);
                        placer(R.items[2], R.dir + 1.5);
                    } else if (get("pusher")) {
                        r.send("33", Math.atan2(_.y - R.y, _.x - R.x))
                    }
                if ( Math.floor(Math.hypot(R.x - _.x, R.y - _.y)) < 150 && Math.floor(Math.hypot(R.x - _.x, R.y - _.y)) > 0) {
                    ...
                    r.send("c", 1, null);
                    if (location.href.includes("sandbox")) place(R.items[2], R.dir - 1)
                    r.send("c", 1, null);
                    if (location.href.includes("sandbox")) place(R.items[2], R.dir + 1.7)
                    r.send("c", 0, null);
                    for (let i = 0; i < Math.PI; i+= Math.PI/8) place(R.items[2], i)
                    if (location.href.includes("sandbox")) place(R.items[2], _.dir);
                    if (location.href.includes("sandbox")) place(R.items[2], null);
        ...
        function ultraspike() {
            if (true) {
                let angle = spikePos(se.currentX, se.currentY, R.dir) // ok
                angle = Math.atan2(angle.x - se.currentX, angle.y - se.currentY) // lul
                placer(R.items[2], angle); // 4x damage wtf
            }
        }
        ...
        setInterval(()=>{
            ...
            if (100 - R.health > 55) {
                storeBuy(6);
                storeEquip(6);
                setTimeout(()=>placer(R.items[0], null),47);
            }
            if (100 - R.health == 50) {
                R.visible = false
                storeBuy(22);
            ...
            if (R.health != 100 && !healing) {
                if (R.health > 85) { // dont die to bull bleeed
                    setTimeout(() => {
                        for (let p = 0; p < R.health / 40; p+=2) placer(R.items[0], null);
                    }, 500);
                } else {
                    healing = true;
                    setTimeout(() => {
                        for (let p = 0; p < R.health / 40; p+=2) placer(R.items[0], null);
                        if (Date.now() - shmae2 < 120) {
                            shmae++;
                            shmae2 = Date.now()
                            healing = false;
                        } else {
                    ...
                    if (
                        ...
                        if ( Math.floor(Math.hypot(R.x - _.x, R.y - _.y)) < 150) {
                            if (Math.floor(Math.hypot(R.x - _.x, R.y - _.y)) > 0) {
                                ...
                                storeEquip(6);
                                if (location.href.includes("sandbox")) placer(R.items[2], R.dir - 1.5)
                                if (canCounter) {
                                    let oldDir = R.dir;
                                    storeEquip(13, !0), storeEquip(7);
                                    Sn(R.weapons[0], true), r.send("c", 1, 90**100)
                        ...
                        if (_.isPlayer) {
                            ...
                            _.health > 0 &&
                                ...
                                2 * o.healthBarWidth * (/*_.healthMov*/ _.health / _.maxHealth),
                                17 - 2 * o.healthBarPad,
                                7
                            ),
                                 be.fill(),
                            R != _ && (be.save(), be.strokeStyle = (R.team != _.team) ? "#cc5151" : "#8ecc51", be.beginPath(), be.moveTo(R.x - f, R.y - d), be.lineTo(_.x - f, _.y - d), be.restore(), be.stroke()));
                        } else {
                            _.health > 0 &&
                                (o.healthBarWidth,
                                 (be.fillStyle = rt),
            ...
            (this.sid = e),
                ...
                if (this.health != this.healthMov) {
                    this.health < this.healthMov
                        ? (this.healthMov -= 10)
                    : (this.healthMov += 10);
                    if (Math.abs(this.health - this.healthMov) < 10)
    ...
    function(e, t) {
        ...
        e.exports = function(e, t, o, c, l, h) {
            ...
            this.getGridArrays = function(e, t, i) {
                ...
                t.map(k => {k.sid == e && (this.disableObj(k))})
